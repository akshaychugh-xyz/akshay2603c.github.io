...
IMPORTANT: Always use absolute URLs for images in posts (e.g., https://akshaychugh.xyz/assets/images/...) instead of relative paths (e.g., /assets/images/...). Relative paths break in newsletter emails. This applies to EVERY image tag — markdown (![alt](url)) and HTML (<img src="url">). No exceptions.
...
BEFORE starting the Jekyll server, run a grep for any relative image paths (e.g., grep for `](/assets/` and `src="/assets/`) in the new post file. If ANY relative image paths are found, convert them ALL to absolute URLs (https://akshaychugh.xyz/assets/...) before proceeding. This is critical — relative paths break newsletter emails.
